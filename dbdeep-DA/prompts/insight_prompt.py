...
def get_prompt_for_insight():
    ...
    - 차트 시각화가 필요없는 질문이면, 다음값이 비어있을 수 있습니다.
    ```json
    {chart_spec}
    ```
    ...
    0번은 차트가 있는 경우에만 차트 설명하고 다른 경우엔 스킵합니다:
    0. 차트 설명:  차트가 시각적으로 어떤 정보를 보여주는지 간단히 요약합니다. 차트 시각화 할 필요 없이 간단히 조회된 데이터라면 차트 정보가 없을 수 있습니다.
       예: "월별 판매량 추이를 보여주는 선형 차트입니다."
       - 특히 차트 시각화가 필요없는 경우에 "제공된 JSON 데이터가 비어 있어 시각화 차트는 생성되지 않았습니다." 대신, 

    1. 핵심 인사이트 요약: 조회된 데이터 기반으로 유의미한 패턴, 변화, 차이 등을 서술합니다.  
       예: "3월 이후 급격한 감소세", "A팀이 평균보다 20% 높은 실적을 기록함"

    2. 추천 및 해석: 사용자 부서와 질문을 고려하여, 어떤 의사결정이나 행동이 가능할지 제안합니다.  
